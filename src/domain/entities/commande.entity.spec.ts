...
describe('Commande Entity', () => {
  describe('calculerTVA', () => {
    it.each([
      {
        type: CommandeType.SUR_PLACE,
        montantHT: 100000,
        expectedTVA: 0,
        description: 'SUR_PLACE sans TVA',
      },
      {
        type: CommandeType.A_DISTANCE,
        montantHT: 100000,
        expectedTVA: 20000,
        description: 'A_DISTANCE avec 20% TVA',
      },
      {
        type: CommandeType.SUR_PLACE,
        montantHT: 50000,
        expectedTVA: 0,
        description: 'SUR_PLACE petit montant',
      },
      {
        type: CommandeType.A_DISTANCE,
        montantHT: 50000,
        expectedTVA: 10000,
        description: 'A_DISTANCE petit montant',
      },
  ...
  describe('calculerMontantTTC', () => {
    it.each([
      ...
      'montantHT=$montantHT, tva=$tva → $expected',
      ({ montantHT, tva, expected }) => {
    ...
    it.each([
      {
        ...
        montantTTC: 100000,
        expected: null,
        description: "SUR_PLACE pas d'acompte",
      },
      {
        type: CommandeType.A_DISTANCE,
        montantTTC: 100000,
        expected: 50000,
        description: 'A_DISTANCE 50% acompte',
      },
      {
        type: CommandeType.A_DISTANCE,
        montantTTC: 200000,
        expected: 100000,
        description: 'A_DISTANCE grand montant',
      },
  ...
  describe('calculerMontantHT', () => {
    it.each([
      {
        items: [{ quantite: 100, prixUnitaire: 300 }],
        expected: 30000,
        description: 'Un seul item',
      },
      {
        items: [
          { quantite: 100, prixUnitaire: 300 },
          { quantite: 50, prixUnitaire: 200 },
        ],
        expected: 40000,
        description: 'Deux items',
      },
  ...
  describe('validerTransition', () => {
    it.each([
      {
        from: CommandeStatut.EN_PREPARATION,
        to: CommandeStatut.PRETE,
        valid: true,
      },
      { from: CommandeStatut.PRETE, to: CommandeStatut.FINALISEE, valid: true },
      {
        from: CommandeStatut.EN_PREPARATION,
        to: CommandeStatut.FINALISEE,
        valid: false,
      },
      {
        from: CommandeStatut.PRETE,
        to: CommandeStatut.EN_PREPARATION,
        valid: false,
      },
      {
        from: CommandeStatut.EN_ATTENTE_ACOMPTE,
        to: CommandeStatut.PRETE,
        valid: false,
      },
      ...
      if (valid) {
        ...
        expect(() => commande.validerTransition(to)).toThrow(
          CommandeStatutInvalideException,
        );
      }
    });
  });
  ...
  describe('validerAcompte', () => {
    it('devrait lancer exception si acompte insuffisant', () => {
      ...
      expect(() => commande.validerAcompte(30000)).toThrow(
        AcompteInsuffisantException,
      );
    });
