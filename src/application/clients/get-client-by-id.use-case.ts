...
export class GetClientByIdUseCase {
  ...
  async execute(id: string, userRole: Role): Promise<Client> {
    const client = await this.clientRepo.findById(id);
    if (!client) throw new ClientNotFoundException(id);

    // Vérifier périmètre
    if (userRole === Role.COMMERCIAL && client.type === ClientType.APPORTEUR) {
      throw new ForbiddenException('Accès refusé à ce client');
    }
    if (userRole === Role.COLLECTEUR && client.type === ClientType.ACHETEUR) {
      throw new ForbiddenException('Accès refusé à ce client');
    }
