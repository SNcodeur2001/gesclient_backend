...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      "Authentifie un utilisateur et retourne les tokens d'accès et de rafraîchissement",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      "Échange un refresh token expiré contre un nouveau token d'accès",
  })
  @ApiResponse({ status: 200, description: 'Nouveau token généré avec succès' })
  @ApiResponse({ status: 401, description: 'Refresh token invalide ou expiré' })
  async refresh(@Body() dto: RefreshTokenDto) {
    const result = await this.refreshTokenUseCase.execute(dto.refresh_token);
    return { success: true, data: result };
  }
